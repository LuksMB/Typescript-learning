...
let meuNome: string[] = ["Lucas", "Maurício", "Braga"];
meuNome.push("LMB");
...
const exemplo: Array<string> = ["Array", "do", "tipo", "string"];
...
let qualquer: any = 2; //tipo number
...
let anyArray: Array<any> = ["Sabão", 1, true, meuNome];
...
setTimeout(function(){
    const teste: string = "Apenas um teste de função anônima..."
...
function mostrarCartao(cartao: {titular: string, saldo: number}){
...
function mostrarVariavel(a: number|string){
    console.log(a);
}

let texto2: number | string = 2;
mostrarVariavel(texto2);
texto2 = "Mudei";
mostrarVariavel(texto2);
console.log("");

//Também é possível validar union types, com condicionais para cada caso em uma função, por exemplo.


//8. Type alias --> permite a criação de tipos mais complexos, como o union type, de forma facilitada.

type tipo1 = number|string;
function mostrarValor(a: tipo1){
    console.log(a);
}
mostrarValor(8);
console.log("");

//9. Interfaces ---> Feito para nomear tipos de objetos de forma mais limpa e organizada.
...
function mostrarCoordenadas(mapa: coordenada){
    console.log("x: " + mapa.x + "\ny: " + mapa.y + "\nz: " + mapa.z);
}

let mapa1: coordenada = {
    x: 5,
    y: 3,
    z: 2
}
...
let textoBase: symbol = Symbol("Teste");
let textoCopiado: symbol = Symbol("Teste");
